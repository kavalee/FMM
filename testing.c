...
double maxErrorRel(double *fast, double *slow, int n) {
    ...
    for (int i = 0; i < n; i++) {
        if (slow[i] != 0) {
            double errRel = fabs(fast[i] - slow[i])/fabs(slow[i]);
            //printf("%lf, ", errRel);
            max = MAX(errRel, max);
        }
    }
...
void generateSpeedData(int N, int num_trials) {
    ...
    fprintf(speed, "Size of Matrix (n),");
    for (int i = 0; i < NUM_PRECISIONS; i++) {
        fprintf(speed, " %d-precision-seconds", PRECISIONS[i]);
    ...
    for (int n = 10; n < N;) {
        printf("%d\n", n);
        double *times = calloc(NUM_PRECISIONS, sizeof(double));
        ...
        fprintf(speed, "%d, ", n);
        for (int i = 0; i < NUM_PRECISIONS; i++) {
            fprintf(speed, "%.4e", times[i] / num_trials);
            if (i != NUM_PRECISIONS - 1) {
                fprintf(speed, ", ");
            }
        }
...
void generateErrorData(int N, int num_trials) {
    ...
    fprintf(error, "Size of Matrix (n),");
    for (int i = 0; i < NUM_PRECISIONS; i++) {
        fprintf(error, " %d-precision-relativeError", PRECISIONS[i]);
        if (i != NUM_PRECISIONS - 1) {
            fprintf(error, ",");
        }
    }
...
void generateFlopsData(int N, int num_trials) {
    ...
    fprintf(speed, "Size of Matrix (n),");
    for (int i = 0; i < NUM_PRECISIONS; i++) {
        fprintf(speed, " %d-precision-flops", PRECISIONS[i]);
        if (i != NUM_PRECISIONS - 1) {
            fprintf(speed, ",");
        }
    }
